...
///
/// This IDE entry point can request completed inference of the expression's scope. Callers
/// already inferring that scope should use `types::definition_resolution` with the expression
/// types available in their inference result instead.
...
mod stub_mapping {
    ...
    use tracing::trace;
    use ty_module_resolver::{ImportingFile, file_to_module, resolve_real_module};
    use ty_python_core::definition::{Definition, DefinitionKind};
...
mod tests {
    ...
    fn builtin_definition_lookup_does_not_infer_scope() -> anyhow::Result<()> {
        ...
        .context("expected definitions for a name expression")?;
